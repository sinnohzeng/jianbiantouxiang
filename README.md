...
- **图标**：图形节里一个开关，图标置顶、文字两行自动缩小适配；可搜 lucide 内置图标、按中文搜 emoji，或上传 SVG / PNG / WebP。内置图标跟随文字颜色与效果，emoji 跨平台取同一份 Noto SVG，上传 SVG 会先做白名单消毒且只在本次会话有效。
...
1. 打开网址，输入文字。主流聊天应用的列表头像大多在 40 像素上下，两到四个字、字重 700 最清楚。桌面上文字与图形一列、配色与质感一列，一屏摊开，不用先切页签。
2. 在“配色”和“质感”两节里挑，或者点“随机颜色”换种子、“随机配色与质感”连质感一起换；改文字随时点操作条上的“文字”快捷入口，一步跳到输入框。
3. 点“导出”立即下载；需要粘贴到聊天窗口时，点导出按钮右侧的设置图标，再选“复制图片”。
4. 要做部门或产品标识，在“图形”节打开开关，选一个内置图标、品牌图形或 emoji，也可以上传自家 logo，再填名称并导出。
5. 字号、行距、边距这类数值都在“微调”里：桌面是右边那一条，手机在页面末尾，收着，需要时展开。每行都能拖滑杆、敲数字，改过的行会出现一个回默认的小按钮。
...
- 着色器、字体选择器、品牌索引都按需加载，首屏 JS 现在 205 KB gzip。

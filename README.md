...
- **图标徽章**：图形在上、文字在下，比例可调；可搜 lucide 内置图标、按中文搜 emoji，或上传 SVG / PNG / WebP。内置图标跟随文字颜色与效果，emoji 跨平台取同一份 Noto SVG，上传 SVG 会先做白名单消毒且只在本次会话有效。
...
4. 要做部门或产品标识，切到“图标徽章”，选一个内置图标或 emoji，也可以上传自家 logo，再填名称并导出。

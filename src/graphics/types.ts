...
export type LucideIconNode = readonly [tag: string, attrs: Readonly<Record<string, string>>]

/**
 * 图形的三种运行时形态。路径保留矢量，导出 8192 时不因预览尺寸先栅格化而发糊；
 * 图片默认保留原色，只做等比 drawImage，品牌标志开了单色档才在绘制期压成剪影。
 */
export type Graphic =
  | {
      kind: 'lucide'
...
/** 画图形。内置图标跟随文字效果；图片类保持原色，品牌单色档除外。 */

/**
 * React Bits `@reactbits-starter/staggered-text-tw`，本仓改动：
 * Transition 与 Easing 改成 import type（verbatimModuleSyntax），
 * buildKeyframes 与 IntersectionObserver 回调按 noUncheckedIndexedAccess 收紧。
 */
...
const buildKeyframes = (
  ...
  keys.forEach((key) => {
    const head = from[key] ?? 0;
    keyframes[key] = [head, ...steps.map((step) => step[key] ?? head)];
  });

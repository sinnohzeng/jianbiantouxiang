/**
 * React Bits `@reactbits-starter/aurora-blur-tw`，本仓改动如下，升级组件时照搬：
 *
 * 一、片元着色器最后一行的 alpha 由颜色亮度推出，不再恒等于 u_opacity。
 * 原版让整块画布按固定不透明度压在页面上，极光之外的区域是黑的，浅色主题会糊出一片脏灰。
 * 二、新增 dpr 与 frameloop 两个透传属性，手机压到 0.5 DPR、标签页不可见时停帧。
 * 三、hexToVec3 与 uniforms 下标取值按 noUncheckedIndexedAccess 收紧。
 */
...
export interface AuroraBlurProps {
  ...
  /** Master opacity (0-1) */
  opacity?: number;
  /** 渲染分辨率倍率，透传给 r3f 的 Canvas。手机传 0.5 省一半像素 */
  dpr?: number | [number, number];
  /** 帧循环，传 "never" 即停帧。标签页不可见时用它把渲染停掉 */
...
void main(){
  ...
  // 亮度即不透明度：极光照不到的地方留给页面自己的底色，浅色主题下不会压出一层脏灰
  float lum=clamp(max(max(c.r,c.g),c.b),0.,1.);
  gl_FragColor=vec4(c,lum*u_opacity);
}
`;

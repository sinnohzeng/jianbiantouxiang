...
逐行字体与字体名预览，规约见 `specs/v8.0-line-fonts-and-preview/`。

### 变更（破坏性）

- **逐行参数收成两组**：`typography` 的 `fontFamily`、`fontSource`、`fontWeight`、`sizeMode`、`fontSize`、`line2Size`、`lineOffsetsX`、`lineOffsetsY` 收成 `line1` 与 `line2` 两组，每组是 `font`、`size`、`offsetX`、`offsetY`。`font` 是 `{ family, source, weight }`；第二行的 `font` 为 `null` 时跟随第一行，`size` 为 `null` 时第一行自动、第二行跟随第一行的 62%。`lineHeight`、`letterSpacing`、边距与效果仍是全局一份
- **存档键升 `gradient-avatar:v4`**：外壳只剩 `{ config, history }`，不带版本号；旧存档整体作废，不迁移，首屏回默认。字体目录缓存键改 `gradient-avatar:font-catalog:v1`，去掉子集与分类两列；最近使用字体改存 `gradient-avatar:recent-fonts`，只记 Google 字体，至多 8 款
- **文字卡片**：独立的字体节与字重节删掉。每行输入下面是一条字体行，左边字体按钮、右边字重下拉，第二行多一颗“跟随”钮；再下面是这一行的字号。字重下拉只列该字体真有的档位，系统字体与上传字体同样吸附到真实档位，查不到时用 300 到 900
- **字体选择器**：桌面是锚在字体按钮正下方的弹层，只在上下两侧翻转，不翻到左右；手机是抽屉，标题写明为哪一行选。分组依次是已上传、最近使用、精选、全部、系统字体；精选组按界面语言排序，繁体组收香港字体
- **字体按行加载**：两行字体不同时并行加载，各自回落、各自提示，提示带字体名；回落中的字体在字体按钮上显示琥珀色告警图标。只改文字、字体没变时只补字形，不出加载条
- **镜像**：jsDelivr 第二档从 gcore 换 fastly

### 新增

- **第一行与第二行分别设字体与字重**：第二行默认跟随第一行，改了第二行的字体或字重才独立，“跟随”钮回到跟随。第二行只改了字重时换第一行字体，第二行跟着换字体、保留自己的字重
- **字体名预览**：选择器里每个字体名、卡片上的字体按钮都用这款字体自己写出来。Google 字体按 css2 的 `text=` 只拉名字那几个字，每款 1 到 4 KB，以 `fp-<id>` 别名注册，不碰画布正在用的字体；行滚进列表才请求，每款整场会话只请求一次，失败留在界面字体。中日韩字体有作者或发行方给出的原生名时显示原生名，后面跟一段西文 family 小字，中日韩精选 47 款里 29 款有原生名，另有苹方、微软雅黑等 4 款系统字体。设计与否决的备选见 ADR-0007
- e2e：第二行字体的弹层位置、字重下拉与跟随钮；手机字体行的热区与抽屉标题；预览子集用本地夹具，断言行就绪后以别名渲染

### 修复

- ADR-0003 里“css2 的 `text=` 对可变字体无效，Noto Sans SC 700 仍返回 4.6 MB”的结论来自非浏览器 UA，用浏览器 UA 复测只回 2 KB 左右。主链路仍不用 `text=`，理由改成按切片缓存能跨文字复用，见 ADR-0007

## [7.1.0] - 2026-09-11

### 变更（破坏性）

- **第二行字号独立**：拖第一行字号时第二行原地不动。`typography.lineSizeScales`（相对第一行的乘数）换成 `typography.line2Size`（画布短边比例，与 `fontSize` 同一单位，`null` 为跟随第一行的 62%）。第二行滑杆与第一行同一单位、下限 2%，带自己的“自动”钮回到跟随；两段之间的留白改按较大的那段字号算，第二行放大过第一行时不贴在一起；第二行还在跟随时拖第一行，先把第二行钉在此刻的大小。求解器仍只搜基准一个自由度，第二行手动时按定值排，定得放不下时只缩第一行并标超框。旧存档手动档的乘数按当时的字号折成短边比例，自动档回到跟随

### 修复

- 部署文档：ESA Pages 的“域名绑定”只收带前缀的主机名，根域名改走“路由”加一条代理加速的根记录，`www` 301 到根域名

## [7.0.0] - 2026-09-10

微调面板拆解、操作条三格、顶栏设置与域名切换，规约见 `specs/v7.0-panel-dissolve/`。

### 变更（破坏性）

- **微调面板拆掉**：数值滑杆按归属散进四张卡片。文字卡片里第一行输入下面是第一行字号、第二行输入下面是第二行字号、文字样式下面是强度，文字颜色收成一行（五个预设加自定义色块，去掉 hex 框与米白、深灰两档），边距、行高、字间距收进折叠的“版面”，每行的水平与垂直补偿收进折叠的“位置微调”；图标卡片磁贴下面是图标大小，再是折叠的“位置微调”；质感卡片种子行之后是折叠的“参数”。`src/app/workspace/Inspector.tsx` 与 `src/app/inspector-open.ts` 删除，localStorage 里的 `gradient-avatar:inspector-open` 不再读取
- **操作条三格**：换一版（只换种子）、随机配色（只换配色，种子与质感不动）、导出加导出选项齿轮。“随机颜色和质感”其实连种子也换，名字与行为对不上，改成只换配色；存储层 `randomizeAll` 改名 `randomizePalette`。微调钮与“更多”菜单删除，长短两版文案与容器查询一并删除
- **顶栏**：删标语；右侧按钮组左邻加备案号 `京ICP备2026029073号-5`，浅色小字链到工信部，手机上移到挑选栏末尾一行；独立的主题按钮并入新的设置菜单，菜单收主题三档、网格与安全区参考线、恢复默认。`about.html` 页脚同样加备案号
- **桌面栅格**：微调那一列没了，六套列模板收成三套（64rem 两列、70rem 三列、96rem 冻 clamp），`--inspector-w` 与 `data-inspector` 删除
- **域名**：线上地址改为 `https://jianbiantouxiang.com`，托管在阿里云 ESA Pages，构建配置在新增的 `esa.jsonc`；旧域名 `jianbian.zixuan.net` 留在 Cloudflare Pages 做 301。`package.json` 的 `homepage`、README、AGENTS.md、`docs/architecture.md` 同步；接入步骤、缓存规则与上线核对命令在新增的 `docs/deploy.md`

### 新增

- **垂直视觉补偿**：`typography.lineOffsetsY`（画布高比例）与 `layout.graphicOffsetY`（安全框高比例），量程与步进照抄水平（±25%，0.25%）。补偿不参与字号求解，不挤压文字可用区，越出安全区只进 `overflow`；旧存档没有新字段时补 0，画面与升级前一致
- **环境光动得看得见**：极光四层速度改 0.36、-0.17、0.24、-0.09（两层反向漂移换视差），整体速度 0.42 提到 1.3，噪声尺度 2.6 提到 2.9，纵向漂移占比降下来避免与垂直淡出耦合成整屏泵动，一、三层强度各降一档补偿时域对比度；CSS 兜底的漂移周期 26 s 收到 16 s、位移 3%/-4% 提到 6%/-7%，三团光晕相位重新错开。只改 uniform 取值，dpr、帧循环与层数不动，GPU 成本持平。无头 Chromium 下 1 秒两帧的平均像素差从 1.86 提到 4.1
- e2e：操作条三格与随机配色只换配色、顶栏备案号、设置菜单切主题与恢复默认、折叠组展开后的滑杆与重置钮、垂直补偿落存档、手机端备案号在页面末尾

### 修复

- 滑杆行数值框比滑杆右端缩进 26 px：列序“标签 | 数值 | 重置占位”把 20 px 占位加 6 px 间距放在了最右，改成“标签 | 重置占位 | 数值”，占位列无条件渲染，没给默认值的行也不会滑进窄列
- 预览画框底下一条被切出来的横线：`preview-pane` 的 `overflow-hidden` 把三层长投影与光晕裁掉了。桌面改 `overflow-visible`，横向溢出由工作台外壳兜住；手机保留裁切，投影收成两层短的、光晕外扩收到区底以内
- 320 px 宽的 iPhone SE 上滑杆行标签被截成省略号、操作条中间格文案截断：手机档数值列 6rem 收到 5rem，数值框与“自动”钮内边距各收一档，标签手机上折行、只在桌面截断；操作条第二格英文改 New palette、日文改 別の配色，第一、二行字号日文改 1 行目サイズ、2 行目サイズ，繁体第二行字号统一用“字級”

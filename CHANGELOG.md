...
v5.0 工作台与债务清理，规约见 `specs/v5.0-workspace/`。§A 已落地，§B 只剩默认配方待定稿；整轮按 major 发布。

### 新增

- **炫技层**：极光着色器背景接当前配色与环境光滑杆，首屏加载动画，挑选栏与微调 stagger 进场，预览框缩放模糊入场与桌面指针跟随的 3D 倾斜高光，分段控件与磁贴的选中描边共享元素滑动，随机与导出成功时的涟漪加预览弹动，顶栏标题逐字入场，数值框平滑计数；`prefers-reduced-motion: reduce` 与 `VITE_SHOWCASE=0` 时整套不挂，回落到原来的 CSS 光晕
- **手机长按直接存图**：触屏上预览里盖着一张按当前配置合成的 JPG，长按它就走系统的保存图片，不用先点导出；预览停稳 600 毫秒后在后台出图，页面切到后台不出，网格与安全区参考线不进图
- **双列挑选栏**：页签与手风琴取消，左列文字与图形、右列配色与质感，两件最常用的事同屏并排；1280 以上两列并排，1024 到 1279 并回一列；画布挪进微调面板
- **微调面板默认收起**：数值滑杆是低频动作，宽度先让给改文字与换配色，需要时从操作条打开，桌面在最右侧多开一列而不是浮在预览上；开合状态落本机，手机与桌面共用同一个开关
...
- **操作条按钮全部带可见文案**：只有图标时没人认得出哪个是哪个，触控目标再大也没用；手机上是图标在上、短文案在下的五格，桌面是图标加全称的一行，横跨工作台底部整宽
...
- 单测 667 条全绿；`npm run e2e` 34 条全绿（新增双列工作台与微调默认收起、预览区无滚动条、更多菜单里的恢复默认、网格留存、字号自动态切手动、存档刷新恢复）

...
- **炫技层**：极光着色器背景接当前配色与环境光滑杆，首屏加载动画，挑选栏与检查器 stagger 进场，预览框缩放模糊入场与桌面指针跟随的 3D 倾斜高光，分段控件与磁贴的选中描边共享元素滑动，随机与导出成功时的星爆粒子，顶栏标题逐字入场，数值框平滑计数；`prefers-reduced-motion: reduce` 与 `VITE_SHOWCASE=0` 时整套不挂，回落到原来的 CSS 光晕
- **手机长按直接存图**：触屏上预览里盖着一张按当前配置合成的 JPG，长按它就走系统的保存图片，不用先点导出；预览停稳 600 毫秒后在后台出图，页面切到后台不出，网格与安全区参考线不进图
- **桌面两层平铺工作台**：页签与手风琴取消，左列文字、图形、配色、质感、画布五节常驻，中间预览，右侧“微调”检查器带按排版、逐行、效果、质感、画布分组；1280 以上三列，1024 到 1279 两列，检查器落到预览下方
...
- 手机上预览的网格与安全区开关缩到 36 像素并挪到画框右下角，不再盖住画面；触控热区仍是 44 像素

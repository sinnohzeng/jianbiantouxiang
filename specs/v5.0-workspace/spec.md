...
状态：已封存。§A、§B 均已落地并随 5.0.0、5.1.0、5.2.0 三个版本发布，与实到实现的差异见文末“实到范围”一节。
本文件不再回写正文，正文是当轮意图快照。
...
布局围绕这个来：挑选类控件占首屏、尺寸大；数值微调收进按需打开的面板。
整体气质要压得住场，动效与着色器能上的都上。体积与首开速度不是约束：这不是搜索首页，多等两三秒也行，
慢就上加载动画；reduced-motion 仍然尊重。

### B1 桌面双列工作台

分列的依据是使用频率：改文字与换配色是最常用的两件事，让它们同屏并排。数值微调按需打开。

- 断点与网格：
  - ≥1280：挑选栏拆两列（文字图形 / 配色质感），右边是预览；微调打开时最右再多一列。
  - 1024 到 1279：挑选栏并回一列，仍与预览左右分；微调打开时同样在最右多一列。
  - <1024：手机纵向栈，见 B3。
  - 预览那一格 `overflow-hidden`，画框按 `--preview-max` 夹一次，不出现滚动条。
  - 操作条横跨工作台底部整宽，不只占预览那一列：六个带文案的按钮挤在一列里必然溢出。
- 页签、手风琴全部取消。挑选栏每节一张卡片，标题常驻：
  1. 文字（左列）：两行输入（高 44px）与第二行右侧的图标开关沿用；效果分段（无 / 描边 / 投影 / 发光 / 胶囊）；
     文字色分段（自动 / 自定）加色块。
  2. 图形（左列）：当前图形磁贴（无图形时是虚线空位），旁边“更换”“清除”；点磁贴或“更换”打开 IconPicker。
     大小与水平补偿两根滑杆在微调面板里。
  3. 配色（右列）：色调分段与色系筛选沿用；配色缩略图是 4 列渐变磁贴（高 56px，名字在下，选中态描边）；
     “随机配色”放大成主按钮；自定义色与种子生成器留在本节末尾、默认折叠。
  4. 质感（右列）：四种质感 2×2 磁贴，每块用当前配色画一小张 CSS 渐变示意。
  5. 画布：不在挑选栏，收进微调面板，形状分段与尺寸预设胶囊沿用，宽高输入保留。
- 微调面板默认收起，从操作条打开，开合状态落 localStorage `gradient-avatar:inspector-open`，手机与桌面共用。
  收起时整块不渲染：Base UI 的滑杆在 `display:none` 里挂载会量到 0 宽并把滑块永久藏掉。
  每组一张卡片，一行“标签加数字框”上、滑杆独占一行下，分组小标题：
  ...
  - 质感：强度、柔和、颗粒、缩放、旋转、高光；
  - 画布：尺寸预设、宽高、形状、圆角（只在形状为圆角时）、导出底色。
  - 每行偏离默认值时出现“重置”小按钮回默认值。
- 最近生成收进顶栏右上角的浮层，与撤销重做同一处落点。
- 操作条：随机颜色（一级实心）、随机配色与质感、文字、微调、更多、导出，每个都带可见文案。
  一键恢复默认在“更多”菜单里，带一次确认。
- 代码落点：新建 `src/app/workspace/`（`PickColumn.tsx` 两列、`Inspector.tsx`、`MobileDivider.tsx`），
...
- 预览仍 sticky 在顶栏下；内容区是挑选栏四节，末尾是默认收起的“微调”。底栏是图标加短文案的五格。
...
- 清单按收益排序，前四项本轮必做：
  ...
  2. 进场编排：挑选栏各节与微调分组 stagger 淡入上浮（间隔 40ms），预览框从 0.96 缩放加 8px 模糊到清晰（500ms）。
  3. 选中态流动：配色磁贴、风格磁贴、分段控件的选中描边用 `layoutId` 共享元素在选项间滑动。
  4. 随机按钮：点击荡一圈纯 CSS 涟漪（`pointer-events-none`，圆角随按钮），预览框 1.02 弹一下。
     着色器粒子在按钮尺度上会露出自己那块方形画布，试过就撤了。
...
§B 落地时六处与本文正文相反，实现以 `CHANGELOG.md` 的 5.0.0 段为准，正文不回写：

- **微调列位**：正文（B1）写微调打开时在最右再多一列；实到是最左一列（`index.css` 的
  `grid-column: 1`）。
- **拆列断点**：正文（B1）写统一 1280 起拆两列；实到分两档，微调收起时 1280 拆、展开时 1536 才拆，
  不够就并回一列。
- **操作条跨度**：正文（B1）写操作条横跨工作台底部整宽；实到只占预览那一列。
- **参考层开关位置**：正文（A3、B1）写网格与安全区开关放在预览角上；实到挪进了底栏“更多”下拉。
- **文字色自动通道**：正文（B1 第 1 节）写文字节保留“文字色分段（自动 / 自定）”；实到自动取色
  整条通道已删，文字色只剩用户挑的那一个。
- **字重与画布落点**：正文（B1 微调面板）写字重在排版组、画布是微调面板里独立一组；实到字重挪到了
  文字节，画布挪进了导出抽屉。

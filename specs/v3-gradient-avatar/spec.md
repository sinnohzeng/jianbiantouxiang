...
- 不支持 WebGL2 的浏览器：显示明确提示，并回落到 CSS 多层 `radial-gradient` 的静态近似（仅预览，不导出）。
...
- 字号模式：
  - 自动填满（默认）：二分搜索字号，让整块文字在安全区内尽可能大；安全区由“边距”参数决定，方形与圆角默认每边 10 %（文字占边长 80 %），圆形默认每边 15 %（文字占 70 %，对应内接区域），切换形状时若用户没改过边距则跟随默认。
...
- 文件名：`<文字>_<宽>x<高>.<ext>`，文字为空时用“渐变头像”。

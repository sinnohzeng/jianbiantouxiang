...
> 2026-08-31 的 v3.1.2 已调整默认文字参数、种子入口、行级字号交互与颗粒形状池；当前产品口径以
> `specs/v3.1.2-defaults-and-grain/spec.md`、`specs/v3.1.1-direct-export-line-controls/spec.md` 与 ADR-0005 为准。
...
- 四种质感（style）：
  ...
  | `grain`（颗粒）      | `grainGradient`      | `frame`、`shape`（`wave` / `corners` 两种，v3.1.2 移除 `ripple`）            |                                                    |
...
- 字号模式：
  - 自动填满（默认）：二分搜索字号，让整块文字在安全区内尽可能大。安全区先按“边距”参数内缩（默认每边 15 %），再按画布形状收：圆形与圆角矩形的遮罩会切掉方框四角，所以把方框按原比例缩到四角正好压在遮罩边界上，方角与常规圆角够不着四角，几何原样不动。边距不随形状变，换形状不改用户设的值。
  - 手动：字号以画布短边百分比表示，范围 4 %–92 %。
- 排版参数（全部可调，全部有合理默认）：行高 0.85–2.0（默认 1.03；单行时行高不影响排版）、字间距 -0.1–0.5 em（默认 0，2 到 4 个 CJK 字的短文字自动填满时用 0.05 em 作为起点）、字重（按当前字体可用字重）、对齐（左 / 中 / 右）、锚点（九宫格）加微调偏移、竖排开关（CJK）。

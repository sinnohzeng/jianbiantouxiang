...
| 智能体   | 目录            | 产出                                                                                                                                                                                                                                                                                                                                                                                                             | 测试                                                  |
| -------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| engine   | `src/engine/`   | `STYLES` 定义（四种 style 的 shader、参数映射、默认值）；`seedToParams(seed, style)`；`createGradientMount(container, config, sizePx)`；`renderGradient(config, w, h): Promise<HTMLCanvasElement>`（离屏 ShaderMount，`speed 0`，`preserveDrawingBuffer`，读 `MAX_RENDERBUFFER_SIZE` 限幅后放大）；`hasWebGL2()`；`cssFallbackStyle(config)`                                                                     | seed 确定性、参数落在合法区间、限幅逻辑               |
...
- `gh repo rename gradient-avatar`，确认 Cloudflare Pages 构建仍触发。

...
description: v5.0 工作台与债务清理（2026-09-05 发布 5.0.0）：补偿独立、字号自动手动无缝、网格参考线、移除 URL 分享与自动文字色的口径，桌面栅格按“还剩多少给画框”定断点，操作条文案走容器查询，样张分三张
metadata:
  type: project
---
...
- 2026-09-05 收尾：owner 看图后要的五件事都做了。布局从「三列常驻检查器」改成「双列挑选栏加按需打开的微调」，
  owner 亲选「默认收起，按需打开」；最近生成挪进顶栏浮层；每个按钮都带文案；
...
- 2026-09-05 发布 5.0.0。第二轮 owner 反馈全部落地：配色去家族下拉与重复随机、深色补到 16 套（共 37）、
  操作条只占预览那一列、文字颜色去掉“自动”整条通道（连带删掉预览的离屏取色探针与自动底板）、
  文字样式默认投影且排第一、图形改叫图标、微调面板挪到最左、环境光去掉滑杆默认全开、
  跟随系统换成半明半暗的圆、画布并入导出设置（默认 2048、2 MB）、悬停变深黑修掉、操作条去掉“文字”入口、
  “更多”菜单塞进复制图片与两条参考线。
- 桌面栅格的判据改成“还剩多少宽度给画框”：微调收起 1280 拆两列，展开要 1536 才拆，不够就并回一列。
  列模板与 `display` 全在 `src/index.css` 的 `[data-slot='workspace']` 一段，
  class 里不能再留 `flex`，工具层排在组件层之后会压掉 `display: contents`。
  操作条文案用容器查询按自身宽度分级收放，不用视口断点：它的宽度随微调开合在 524 到 1696 之间跳。

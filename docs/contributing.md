...
- 临时文件只写会话草稿目录，不往仓库根目录落任何文件；不关闭共享的浏览器，也不动别人的快照。派活的 prompt 里把这两条写明，并行调研时尤其如此。
- 实现切片派给子智能体时，要求每次工具调用小而快；中途卡住就用原智能体续跑，不重开。
- 跑 e2e 或截图前先看 4173 端口上的预览服务是不是这次 build 之后起的，不是就停掉，让 Playwright 自己起。

...
视觉顺序由 CSS grid 的行列指定，DOM 顺序按手机排，预览、分隔条、挑选栏两列、微调、操作条。

拆不拆两列，判据是还剩多少宽度给画框，不只是视口有多宽：微调那一列要吃掉近三百像素，
1456 以下四列并排会把挑选栏压到三百出头，比不拆还糟。
...
| 断点 | 形态 |
| --- | --- |
| ≥1456，微调打开 | 微调、文字图标、配色质感、预览四列并排 |
| ≥1120，微调收起 | 挑选栏拆两列（文字图标 / 配色质感），右边是预览 |
| 1024 到上一档之间 | 挑选栏并回一列；壳宽过 480 px 时壳内自己拆两列；微调打开时它在最左多开一列 |
| <1024 | 纵向栈：预览 sticky 在顶栏下，下面一条可拖的分隔条，再往下是挑选栏四节与收起的“微调” |

列模板与落位全在 `src/index.css` 的 `[data-slot='workspace']` 一段，断点乘开合共六套；
`display` 也归那里管，工具层排在组件层之后，class 里留一个 `flex` 就会压掉 `display: contents`。
壳内拆两列用有界 media 查询而不是容器查询：壳在拆列档是 `display: contents`，不生成盒，
承担不了 `container-type`；壳宽又完全由视口断点乘微调开合决定，容器查询量不到新信息。
...
桌面上它是两行三列 grid：换一版、随机颜色和质感、微调在第一行，更多与导出在第二行。
预览列只有四百多像素宽，一行五颗带字按钮必然截断，而预览 pane 竖向有富余。
五个按钮一律带字，没有只剩图标那一档，图标认不出来的按钮用户得点一次才知道是干嘛的。
全部随机那一颗有长短两版文案，都在 DOM 里，露哪一版由容器查询按操作条自己的宽度定
（30rem 以上给全称，手机通栏五格给短称），而不是视口断点；换一版长短同文，单 span 恒显。
span 上不能挂 `hidden` 这类工具类，工具层排在组件层之后会把容器查询的 `display` 压掉。
手机上它固定在屏幕底并让出 safe-area，图标在上、短文案在下的五格。

挑选栏分列的依据是使用频率：改文字与换配色是最常用的两件事，让它们同屏并排，不用先切页签也不用滚一屏。
左列是文字与图标，右列是配色与质感。没有页签，没有手风琴，全站只剩自定义配色、种子色生成与“微调”三处折叠。
...
微调面板收全部数值滑杆，分排版、图标、效果、质感四组（逐行的三行并进排版组），每组一张卡片。
每行 38 px 两段：第一段是「标签、自动档、数值框、重置占位」的固定列 grid，第二段滑杆独占一行。
数值框与重置占位各占定宽列，位置与兄弟元素有无无关——早先第一行用 justify-between，
字号行多一颗自动钮又没有重置占位，它的数值框比其余行右移 28 px，同一组竖着看是歪的。
致密尺寸一律 `lg:` 前缀：手机渲染同一棵树，基值要保持 44 px 触控与 16 px 输入字号的口径。
用不上的组不出现（没有图标就没有图标组）。桌面无须折叠头：它与操作条的微调钮是同一个开关的两个入口。
画布尺寸与形状不在这里，它们跟格式与体积是同一件事，收在导出抽屉里。
默认收起，开合状态在 `src/app/inspector-open.ts`，落 localStorage `gradient-avatar:inspector-open`，手机与桌面共用。
...
`src/engine/styles.ts` 是引擎里唯一知道 shader uniform 名字的地方。微调面板只认 `styleParams` 的五个滑杆，渲染层只认 `StyleRenderPlan`，换 shader 或调区间都只动这一个文件。
...
预览参考层（安全区、网格）的开关与手机预览高度一样是“怎么看”而不是“出什么图”：各自是模块级状态加 localStorage（`gradient-avatar:overlays`、`gradient-avatar:preview-height`），不进 `AvatarConfig`，导出永远不画。两个参考层的开关收在操作条的“更多”菜单里，带文案与勾选态，不再摆在画框角上挡住要看的那一块。网格是 CSS 渐变画的 DOM 图层，放在着色器宿主之外，格子边长取画框短边的 1/12，从中心铺开，中心十字加粗，白色低透明度加 `mix-blend-mode: difference`，深浅底都可辨。
...
| 端到端 | `npm run e2e` | 两个 project：`desktop` 跑 1440 桌面，`iphone-15` 跑设备模拟。覆盖内置图标、emoji、品牌图形、品牌单色切换、上传 SVG、双列工作台与微调默认收起、桌面首屏四节与默认微调组不溢出且画框不超 640、预览区无滚动条、微调重置钮与数值框对齐、图标移除钮清空回空态、更多菜单里的恢复默认、手机分隔条拖拽留存、手机底部抽屉、存档刷新恢复、网格开关留存、字号自动态切手动、关于页独立成页、赞赏区渲染、炫技层背景、微信长按保存与既有导出路径 |

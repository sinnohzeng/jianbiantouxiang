...
拆不拆两列，判据是还剩多少宽度给画框，不只是视口有多宽：微调那一列要吃掉三百像素，
1280 上四列并排会把画框压到 300，比不拆还糟。

| 断点 | 形态 |
| --- | --- |
| ≥1536，微调打开 | 微调、文字图标、配色质感、预览四列并排 |
| ≥1280，微调收起 | 挑选栏拆两列（文字图标 / 配色质感），右边是预览 |
| 1024 到 1279，或微调打开且 <1536 | 挑选栏并回一列；微调打开时它在最左多开一列 |
...
微调面板收全部数值滑杆，分排版、逐行、图标、效果、质感五组，每组一张卡片，
一行“标签加数字框”上、滑杆独占一行下。用不上的组不出现（没有图标就没有图标组）。
...
桌面上由外壳给出 `--preview-max`，也就是这一列留给预览的净空，画框取 `min(var(--preview-max), 720px)`。

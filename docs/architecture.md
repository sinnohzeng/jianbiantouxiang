...
| `layout` | 图形比例与图形来源；v4 起无用途分派，图标、第一行、第二行一个纵向栈 | 文字排版、图形、URL |
...
每一行可以按 `lineOffsetsX` 做水平视觉补偿：落位是纯位移，改第 i 行只动第 i 行，求解阶段按「安全区宽 − 2 × |补偿| × 画布宽」预留余量。次行字号乘 `lineSizeScales[1]`；绘制层按行设置 `ctx.font`，描边、投影、发光的尺度也随之按行走。文字面板把行级控件放在全局字号之前。第一行为空、第二行有内容是合法槽位（图标加说明文字），空槽位留住，补偿参数跟着内容走。

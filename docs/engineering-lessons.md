...
微调行第一行早先是 `justify-between`：左边标签，右边「自动钮 + 数值框 + 重置占位」。
全仓只有字号行有自动钮、又只有它没有重置占位，于是它的数值框比其余行右移 28 px，
同一组竖着看是歪的——两端对齐把右缘钉死，中间多一块少一块全由内容决定。
解法是第一行改固定列 grid（`minmax(0,1fr) 36px 56px 20px`），自动钮住进标签那一格，
数值框与重置占位各占定宽列，位置与兄弟元素有无无关。判据也简单：
量同一组里所有数值框的左缘，差值必须是 0。

### 预览列用 1fr 捡漏，代价是控制面板滚出首屏

旧栅格里预览列吃剩余宽度：1920 上画框 890 px，三列控制面板被压到 400，
配色加质感那一列内容 1615 px、可视 812 px，最高频的两个操作整组在滚动区里。
改成预览按视口定宽（`clamp(400px, 34vw, 640px)`）、挑选栏吃剩余之后，
八种视口乘开合的组合溢出全部归零。还有一层坑：`vw` 永远对真实视口取值，
而工作台外壳被 `max-w-[1520px]` 钉死，1647 px 以上 clamp 会继续涨而容器不涨，
挑选列被压出 `minmax` 下限、超出部分被 `overflow-hidden` 裁掉——
超宽档必须把 clamp 冻成字面值。量布局别只看截图，量 `scrollHeight - clientHeight`。

### display:contents 的元素承担不了 container-type

想给挑选栏壳加容器查询按壳宽内拆两列，实测不成立：壳在拆列档是 `display: contents`，
不生成盒，而 `container-type` 的 containment 作用于元素的 principal box。
退一步想，壳宽完全由视口断点乘微调开合决定，容器查询量不到任何新信息，
有界 media 查询（`min-width` 乘 `max-width`）才是诚实的工具。
容器查询该用在宽度真由内容或父级剩余决定的地方，比如操作条的长短文案。

### 截图脚本睡固定毫秒，目检会看到动画半路的假故障

`scripts/screenshots.mjs` 原先 `waitForTimeout(600)` 就截，撞上进场编排时
卡片还在半路平移，手机档整页看起来像横向溢出坏了，量 `scrollWidth` 却完全正常。
目检的等待条件要和 e2e 的 `waitReady` 同一口径：等幕布 `[data-slot="preloader"][data-loading="true"]`
脱离 DOM 再等一拍。截图是给人看的证据，证据本身不能带运动模糊。

## 关于页上线与滚动条收敛（v5.2，2026-09-05）

### Pages 给 `/about` 写 200 重写会撞成死循环

`_redirects` 里写 `/about  /about.html  200`，线上直接 `ERR_TOO_MANY_REDIRECTS`。原因是 Cloudflare Pages 的静态资源层自带一条 HTML 规范化：只要 `about.html` 存在，请求 `/about.html` 一定 308 到 `/about`。这条规范化对重写之后的内部路径同样生效，于是 `/about` → 重写成 `/about.html` → 308 回 `/about`，自己咬自己。

正确写法是**什么都不写**。实测（`wrangler pages dev dist` 逐条验证）静态资源在 `_redirects` 的通配兜底之前解析，`/*  /index.html  200` 不会吃掉真实存在的 `about.html`，`/about` 由资源层自己映射过去。SPA 兜底那条留着，多页站点也照常工作。

线上路由这类问题不要靠推理和一次部署去试，`wrangler pages dev dist` 跑的就是同一套资源层逻辑，本地能完整复现死循环，也能在推之前确认修好了。

### 滚动条只用标准属性，两套写一起必有一套是死代码

MDN 写明：Chromium 里只要元素的 computed `scrollbar-width` 不是 `auto`，`::-webkit-scrollbar` 那一整套伪元素在这个元素上整个作废。所以“标准属性给 Firefox、webkit 伪元素给 Chrome”的双写法在 Chromium 上是自相矛盾的，只能二选一。选标准属性：`scrollbar-width` / `scrollbar-color` 自 Chrome 121、Edge 121、Firefox 64、Safari 18.2 起可用，代价是滑块不能自己定圆角。

两条继承规则不一样：`scrollbar-color` 继承，`scrollbar-width` 不继承。所以颜色挂 `:root` 全站一致，宽度得逐个挂（`* { scrollbar-width: thin }`）。写 hover 高亮时必须在每个滚动容器上把静息色写回去，否则 `:hover` 沿祖先链一路成立，外层壳一亮，颜色会继承给里面每一个子列，碰一下两条全亮。

真正让原生滚动条显得突兀的不是宽度，是那条从头贯到底的不透明轨道。轨道给 `transparent` 是这次改动里效果最大的一步。

### macOS 的悬浮滚动条会让视觉核查看不到东西

本机 Chromium 默认走 macOS 的悬浮滚动条，滑块只在滚动时画出来，静息态截图上什么都没有，把滑块染成纯红也照样看不见。用户那边把系统设成了始终显示滚动条，看到的是常驻的经典滚动条，两边根本不是同一个渲染路径。

核查这类样式要显式加 `--disable-features=OverlayScrollbar` 启动，才能截到用户实际看到的那一档。另外 `scrollbar-gutter: stable` 在悬浮滚动条下按规范是不生效的，但 Chromium 配 `thin` 时仍然占位，别拿它当判断依据。

### 收款码只裁不重画，裁完要解码比对

重新生成二维码要先解码再编码，中间读错一位，码就指向别处或者失效，而这种错在页面上完全看不出来，只有真有人付钱才暴露。裁剪保留原始模块，风险不对称，选裁剪。微信个人收款码正中那块头像还是靠纠错容量盖上去的，重画也复原不了。

裁的时候两个坑：一是按亮度找“黑模块”会把整张海报圈进去，支付宝蓝的亮度只有 105，必须同时卡饱和度；二是找首尾行会把海报上下的标题和 logo 一起圈进来，而微信码正中的头像又会让若干行密度掉到阈值以下，得取最长的一段连续密集区并且允许小空隙并段。静区自己重画成纯白，不要从海报上多取一圈——支付宝那张白卡比静区窄，取过去会把蓝底和姓名带进来。

裁完必须验证：用 jsQR 把原图和成品各解一遍，比对 payload 逐字一致，再落盘。

## 工作台与债务清理（v5.0，2026-09-05）

### 「补偿只是位移」要贯彻到求解器，不能只贯彻到落位

v4.0 把补偿改成落位纯位移之后，回归用例只盖了 `manual` 档，就当问题解决了。可 `auto` 档下求解器还在按 `安全区宽 − 2 × |补偿| × 画布宽` 预留余量，两行共用一个基准字号，第一行一动余量就缩，基准字号跟着缩，第二行看起来就是「被第一行影响了」。`manual` 档同样中招：第一行贴满安全区时余量一扣就折行，块高一涨把第二行推下去。规则是：一个「只动自己」的参数不能出现在任何共享量的公式里，求解、换行、包围盒都算在内；回归用例要覆盖每一种会共享量的模式。

### 派生值进 ui，不进 config

「自动字号拖一下变手动、起点就是自动值」需要面板知道预览刚算出来的字号。把它写进 `typography.fontSize` 会污染存档与撤销栈，每一帧排版都变成一次配置变更。落点是 store 的 `ui.autoFontSize`：预览排版后回写，只在值变化超过 1e-4 时写，面板只订阅这一个字段。`TextLayout.fontRatio` 直接从求解器带出比例，不要拿 `fontSizePx / 短边` 反推，主行的行级比例未必是 1。
...
分段控件的选中格悬停时文字变成一片看不清的深色，深浅两套主题都中招。原因不在配色，在生成顺序：
`peer-checked:text-primary-foreground` 与 `peer-hover:text-foreground` 权重相同，Tailwind v4 把 hover 那条排在后面，
于是悬停选中格时文字被改回 `foreground`，压在实心主色底板上就是深色压深色。
同权重下顺序即胜负，靠调 class 的书写顺序没用。修法是把条件写进选择器本身：`[.peer:hover:not(:checked)~&]:text-foreground`，
让提亮只落在未选中的格子上。凡是“某状态下换色”的规则，都要问一句它会不会被另一个同权重变体盖住。

### 工具类层排在组件层之后，把 display 挪进 CSS 要连 class 一起挪

把挑选栏的 `xl:contents` 从 className 挪进 `index.css` 的 `@layer components`，两列当场并回一列，还没有任何报错。
元素上仍留着 `flex`，而工具层整体排在组件层之后，`display: flex` 直接压掉了 `display: contents`。
一条属性只能有一个主人：要在 CSS 里控制 `display`，就得把 class 里的 `flex`、`grid` 一并删掉，由 CSS 给出基础值。
布局属性从 class 搬进样式表时，先 grep 这个元素上还有没有同属性的工具类。

### 尺寸条件不是视口宽度时，用容器查询

操作条落在预览那一列里，同一个 1440 的视口，微调收起时它有 696 宽，展开时有 736，到 1536 上展开反而只有 524。
按 `2xl:` 这类视口断点切文案，必然在某几档上要么截字要么留白。改成 `container: bar / inline-size` 加 `@container`，
让它按自己有多宽决定露多少字，收起顺序按分量倒着来。判据一句话：这个元素的宽度是不是只由视口决定，
不是的话视口断点就给不出正确答案。

### 断点的判据写成“还剩多少给主角”，不是“视口多宽”

微调面板展开时要吃掉三百像素。原来的规则是“1280 起拆两列”，于是 1280 上四列并排，画框被压到 300，
比不拆还糟。改成两条：收起时 1280 拆，展开时 1536 才拆，不够就并回一列。
布局断点该由“主角还剩多少”反推，而不是先定一个视口数再看主角剩多少。
七档视口乘开合两态实测一遍再定数，比照着设计稿猜一个数可靠。

### `overflow: hidden` 仍然能被脚本滚动，断言要盯意图

“预览区不出现滚动条”原来断言 `scrollHeight <= clientHeight`。给画框加了一团故意探出边界的光晕之后，
这条断言差 1 像素就红了，可预览区是 `overflow: hidden`，永远不会出现滚动条。
后来改成设 `scrollTop = 40` 再读回，仍然是 1：`overflow: hidden` 的盒子照样是滚动容器，只是不画滚动条。
最终断言改成两条：`overflowY` 是 hidden，且画框连同上下留白落在预览区之内。
装饰层故意越界时，断言要盯的是“用户会不会看到滚动条、画面有没有被切掉”，不是某个几何数字。
...
- 改默认值时同步迁移测试口径：测试自动取色就显式 `colorMode: 'auto'`，测试统一字号就显式给 `line2Size` 与 `fontSize` 相同的值。默认值是产品行为，不是测试夹具的隐形前提。
- 行级排版只存比例，不存像素。`fontSize` 与 `line2Size` 都是画布短边比例，`lineOffsetsX` 用画布宽度比例，同一配置在 512、1024、2048 下自然等比；自动填满则必须重新按每行字号度量整块包络，不能先按统一字号求完再“事后缩放”。
...
## v5.3 文档治理与品牌单色（2026-09-05）

### 测试超时与断言超时是两条独立预算

Playwright 的 `test.setTimeout` 与断言的 `timeout` 互不相干，测试超时会直接终止测试，不等断言用完自己那份。仓里六处把两者都设成 60 秒，结果是断言永远拿不满预算，报出来的错一律是 `Test timeout exceeded`，真正的失败点被盖住。CI 上连红六轮查不出原因，一半是因为错误信息在撒谎。规则：外层预算必须严格大于它内含的最大内层预算，`e2e/helpers.ts` 的 `PROBE_TEST_TIMEOUT_MS` 就是按这条算出来的。

### Playwright 的 worker 按需加载测试文件，跑起来之后源码目录是只读的

一次全量 e2e 跑到一半时改了 `e2e/helpers.ts`，后启动的 worker 读到改到一半的版本，五条用例挂在完全指向别处的现象上（报的是找不到上传控件）。测试文件不是启动时一次性快照的。规则：e2e 跑起来之后不碰 `e2e/` 与 `src/`，要改就先停。

### 素材源的覆盖缺口，先问能不能变成计算问题

品牌图标里 58 个只有 14 个有上游单色稿。补素材是维护 44 份额外资源且随上游变，改成绘制期用离屏画布加 `source-in` 压平着色则是一段十行代码，还顺带把“跟随文字色”这个更好的行为白送了。代价要说清：alpha 压平会丢原色稿内部的镂空，所以有官方稿的仍优先用官方稿。

### 检查器行挤三段会把滑杆压到 0 宽

Base UI 的滑杆被压到 0 宽之后会被永久隐藏，键盘与拖动全失灵；用 `display: none` 挂载同理，之后再显示出来也不会复测宽度。布局上给滑杆留最小宽度，不要指望它自己恢复。

### 样张配色分三张排是硬约束

37 套配色挤成两张时第二张高度过万像素，软件渲染下 Chromium 直接报 `Unable to capture screenshot`。`npm run samples` 的分张数是被截图能力卡住的，不是排版偏好。
